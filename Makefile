...
all: $(TRG)
	@echo -e "$(GREEN)✓ Build complete [$(BUILD_TYPE)$(GREEN)]$(RESET)"

$(TRG): $(OBJS) | $(BLD_DIR)
	@echo -e "$(YELLOW)🔗 Linking$(RESET) $(BOLD)$@$(RESET)"
	@$(CC) $(OBJS) -o $@ $(LDFLAGS)
...
clean:
	@echo -e "$(RED)🧹 Cleaning build artifacts$(RESET)"
	@rm -rf $(BLD_DIR)
	@rm -rf .cache
...
install: $(TRG)
	@echo -e "$(CYAN)📦 Installing$(RESET) $(BOLD)$(PRJ)$(RESET) to /usr/local/bin"
	@install -d /usr/local/bin
	@install -m 755 $(TRG) /usr/local/bin/
	@echo -e "$(GREEN)✓ Installation complete$(RESET)"

uninstall:
	@echo -e "$(RED)🗑️  Uninstalling$(RESET) $(BOLD)$(PRJ)$(RESET)"
	@rm -f /usr/local/bin/$(PRJ)
	@echo -e "$(GREEN)✓ Uninstall complete$(RESET)"

run: $(TRG)
	@echo -e "$(GREEN)🚀 Running$(RESET) $(BOLD)$(PRJ)$(RESET)"
	@echo -e "$(YELLOW)─────────────────────────────────────$(RESET)"
	@./$(TRG)

release:
	@echo -e "$(BOLD)$(MAGENTA)🎯 Building RELEASE version$(RESET)"
	@$(MAKE) --no-print-directory RELEASE=1

debug:
	@echo -e "$(BOLD)$(CYAN)🐛 Building DEBUG version$(RESET)"
	@$(MAKE) --no-print-directory

init:
	@echo -e "$(CYAN)📥 Initializing submodules$(RESET)"
	@git submodule update --init --recursive
	@echo -e "$(CYAN)🔍 Generating compile_commands.json$(RESET)"
	@bear -- make
	@echo -e "$(GREEN)✓ Initialization complete$(RESET)"

analyze:
	@echo -e "$(MAGENTA)🔬 Running static analysis$(RESET)"
	@$(CC) $(COMMON_CFLAGS) $(SECURITY_FLAGS) $(DEBUG_CFLAGS) $(INCLUDES) \
	      -fanalyzer -fsyntax-only $(SRCS)
	@echo -e "$(GREEN)✓ Analysis complete$(RESET)"

tidy:
	@echo -e "$(MAGENTA)🧼 Running clang-tidy$(RESET)"
	@clang-tidy $(SRCS) -- $(COMMON_CFLAGS) $(SECURITY_FLAGS) $(DEBUG_CFLAGS) $(INCLUDES)
	@echo -e "$(GREEN)✓ Tidy complete$(RESET)"

valgrind:
	@echo -e "$(BOLD)$(YELLOW)🔍 Building for Valgrind$(RESET)"
	...
	@echo -e "$(GREEN)✓ Valgrind complete, check valgrind-out.txt$(RESET)"
